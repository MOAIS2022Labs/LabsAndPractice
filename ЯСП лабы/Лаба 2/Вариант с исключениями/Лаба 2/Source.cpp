...
#include "Time.h"
#include <Windows.h>
// TODO: finally для закрытия файла в обработке и прочая возня с исключениями...
...
void read_console_str(std::string& str)
{
	read_and_check(str, "Введите время в строковом представлении: ");
}
...
void action(Time& time)
{
	...
	do
	{
		...
		case 3: // перевести время в секунды
			switch (output_menu())
			{
			case 1:
			...
			break;
		case 4: // перевести время в минуты
			switch (output_menu())
			{
			case 1:
			...
			break;
		case 5: // вывести время на консоль
			console_output(time);
			break;
		case 6: // вывести время в файл
		{
			std::string fname = choose_out_file_name();
		...
		case 7: // перевести время в строковое представление
			switch (output_menu())
			{
			case 1:
			...
			break;
		case 8: // сложение двух моментов времени
		{
			Time new_time;
		...
		case 9: // сложение секунд и времени
		{
			uShort sec;
			switch (input_menu())
			{
		...
		case 10: // разность двух моментов времени
		{
			Time new_time;
			initialization(new_time);
			Time res = time - new_time;
		...
		case 11: // разность времени и секунд
		{
			uShort sec;
			switch (input_menu())
			{
		...
		case 13: // проверка на неравенство
		{
			Time new_time;
			initialization(new_time);
			switch (output_menu())
		...
		case 16: // первое больше или равно второму
		{
			Time new_time;
			initialization(new_time);
			switch (output_menu())
		...
		case 17: // первое меньше или равно второму
		{
			Time new_time;
			initialization(new_time);
			switch (output_menu())
		...
		case 18: // разница между двумя моментами времени
		{
			Time new_time;
			initialization(new_time);
			switch (output_menu())
