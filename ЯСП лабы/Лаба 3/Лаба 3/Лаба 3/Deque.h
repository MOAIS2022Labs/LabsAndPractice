...
private:
	...
	T* data;
	size_t head, tail;
	size_t capacity; // размер выделенной пам€ти
	size_t size; // фактический размер

	// увеличение выделенной пам€ти в 2 раза
	void extend();
...
public:
	...
	// получение значени€ из начала
	T front();
	//получение значени€ из конца
	T back();
...
{
	if (head + 1 <= 1) // 0-€ €чейка зарезервирована
		extend();
	data[head--] = value;
	++size;
}
...
{
	if (!size)
		throw std::out_of_range("ѕопытка извлечени€ элемента из пустого дека!\n");
	return data[head + 1];
}
