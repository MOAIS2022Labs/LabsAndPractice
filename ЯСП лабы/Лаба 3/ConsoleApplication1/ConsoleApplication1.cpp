...
void check_value(Vector<X> &vector, X &elem)
{
	...
	// если struct
	else if (!std::strcmp(typeid(vector.get_type()).name(), "struct HumanParameters"))
	{
		std::cout << "Введите возраст, рост и вес: ";
...
int main_menu()
{
	...
	{
		std::cin >> choice;
		if (std::cin.fail() || std::cin.peek() != '\n' || choice < 0 || choice>11) //проверяем что пользователь вводит числа типа int от 1 до 3
		{
